...
export function prepareSubmissionData(
  ...
  // Convert single submission_type to array for junction table
  const submissionTypesData = values.submission_types
    ? values.submission_types.map((id: string) => ({ id: parseInt(id) }))
    : [];
